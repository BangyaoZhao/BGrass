...
```{r}
BV_chain1 = new_BV_chain(A, V, X, L, epsilon = Inf, n_thin = 2)
BV_chain1 = update(BV_chain1, n_ite = 1000)

```
```{r}
BV_chain2 = new_BV_chain(A, V, X, L, epsilon = 0.01, n_thin = 2)
BV_chain2 = update(BV_chain2, n_ite = 1000)
...
```{r}
delta_hat1 = coef(BV_chain1)$delta
delta_hat2 = coef(BV_chain2)$delta
delta_hat1
delta_hat2
```
...
```{r}
beta_hat1 = coef(BV_chain1)$beta_marginalized
beta_hat2 = coef(BV_chain2)$beta_marginalized
